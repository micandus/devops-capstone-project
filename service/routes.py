...
def list_accounts():
    ...
    """
    app.logger.info("Request = List all accounts")

    to_return = Account.all()
    account_list = [account.serialize() for account in to_return]
...
def read_account(id):
    ...
    """
    app.logger.info(f"Request = Return account with id {id}")

    to_return = Account.find(id)
    if not to_return:
        abort(status.HTTP_404_NOT_FOUND, f"Account with id {id} can not be found")
    return to_return.serialize(), status.HTTP_200_OK
...
def update_account(id):
    """
    Update an Account
    Endpoint updates an account with the given data
    """
...
def delete_account(id):
    """
    Delete an Account
    Endpoint deletes an existing account
    """

    app.logger.info(f"Request to delete Account with id {id}")

    to_delete = Account.find(id)
    if not to_delete:
        abort(status.HTTP_404_NOT_FOUND, f"Account with id {id} can not be found")
